...
/* Execute a command. Cambiado el codigo de la funcion del TWRP 2.6 original
 por el codigo de la funcion 2.5, ya que no copiaba el boot */
...
//Comentada la nueva funcion del TWRP 2.6
/*int TWFunc::Exec_Cmd(string cmd, string &result) {
   int fd[2];
   int ret = -1;
   if(pipe(fd) < 0)
